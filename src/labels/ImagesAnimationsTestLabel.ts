import { ChoiceMenuOption, ChoiceMenuOptionClose, ImageSprite, Label } from "../../../pixi-vn/src/classes";
import ImageContainer from "../../../pixi-vn/src/classes/canvas/ImageContainer";
import { FadeAlphaTicker, MoveTicker, RotateTicker, ZoomTicker } from "../../../pixi-vn/src/classes/ticker";
import { CANVAS_APP_GAME_LAYER_ALIAS, Pause, Repeat } from "../../../pixi-vn/src/constants";
import {
    addImage,
    moveIn,
    moveOut,
    ...
    zoomIn,
    zoomOut,
} from "../../../pixi-vn/src/functions";
import { pushIn, pushOut } from "../../../pixi-vn/src/functions/canvas/canvas-transition";
import { addImageCointainer } from "../../../pixi-vn/src/functions/canvas/image-container-utility";
...
export const imagesAnimationsTest = new Label(IMAGE_ANIMAIONS_TEST_LABEL, [
    async () => {
        ...
        narration.dialogue = {
            character: juliette,
            text: `These are my 4 puppets: ${eggHeadName}, ${flowerTopName}, ${helmlokName} and ${skullyName}. They can appear, disappear and animate at my will.`,
        };
...
- ${skullyName} will disappear with a fade effect, wait for 0.5 seconds, and reappear with a fade effect.`,
    ...
    async () => {
        ...
        if (eggHeadOld) eggHeadOld.alpha = 0;
...
- ${skullyName} will zoom in with a speed of 1 and a limit of 50, wait for 0.5 seconds, and zoom out with a speed of 30 and a limit of 10.`,
        ...
        let eggHead = canvas.find<ImageSprite>("eggHead");
        if (eggHead) eggHead.scale.set(2);
        let helmlok = canvas.find<ImageSprite>("helmlok");
        if (helmlok) helmlok.anchor.set(0.5);
        canvas.addTicker(
            "eggHead",
